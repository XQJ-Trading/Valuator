...
class ReActEngine:
    ...
    def __init__(
        ...
        self.max_retries = max_retries if max_retries is not None else config.react_max_retries
        ...
        try:
            ...
            while state.should_continue():
                ...
                if self._detect_infinite_loop(state):
                    logger.warning("Detected potential infinite loop - forcing completion")
    ...
    async def solve_stream(
        ...
        try:
            ...
            while state.should_continue():
                if self._detect_infinite_loop(state):
                    logger.warning("[stream] Detected potential infinite loop - forcing completion")
                    await self._force_completion(state)
                    break
            ...
            # Final answer event
            yield {"type": "final_answer", "content": state.final_answer, "success": state.is_completed and not state.error}
    ...
    def _should_provide_final_answer(self, state: ReActState) -> bool:
        ...
        if "<next_task_required/>" in all_content:
            logger.info("Found <next_task_required/> marker - continuing with next step")
            return False
        
        # Check for explicit completion markers (highest priority)
        explicit_completion_markers = [
            "<final_answer_ready/>"
        ]
        
        for marker in explicit_completion_markers:
            if marker in all_content:
                logger.info(f"Found explicit completion marker: {marker} - triggering final answer")
                return True
        
        # Enhanced completion indicators (fallback)
        strong_completion_indicators = [
            "problem is solved", "task is complete", "calculation is finished", 
            "answer is", "final result", "completed successfully",
            "all steps are done", "process is finished", "ready for a new query",
            "awaiting a new task", "no further steps", "definitively solved",
            "i have confirmed", "i have determined", "the conclusion",
            "based on the performed", "all necessary checks", "sufficient information"
        ]
        
        # Check for strong completion signals
        completion_count = sum(1 for indicator in strong_completion_indicators 
                             if indicator in all_content)
        
        if completion_count >= 2:  # Multiple completion indicators
            logger.info(f"Found {completion_count} completion indicators - triggering final answer")
            return True
        ...
        if recent_observations:
            ...
            if ("no tool was executed" in obs_content and 
                "expected" in obs_content and
                len(recent_observations) >= 3):
                ...
                if consecutive_no_tool >= 3:
                    logger.info("Detected multiple consecutive 'no tool executed' - forcing completion")
                    return True
            
            # If observation indicates we need more steps, continue
            continue_indicators = [
                "need to", "should", "next step", "continue", "more data needed",
                "additional information", "further analysis", "will check",
                "let me", "i will"
            ]
    ...
    def _is_repetitive_completion_pattern(self, state: ReActState) -> bool:
        ...
        completion_phrases = [
            "problem is solved", "task is complete", "ready for a new query",
            "no further steps", "already provided the final answer"
        ]
    ...
    def _detect_infinite_loop(self, state: ReActState) -> bool:
        ...
        if len(action_contents) >= 5:
            ...
            for i in range(len(action_contents) - 1):
                if self._similarity_score(action_contents[i], action_contents[i+1]) > 0.9:
    ...
    async def _action_step(self, state: ReActState):
        ...
        action_prompt = self.prompts.format_action_prompt(state, tools_info, current_datetime=current_time)
...
The analysis reached the maximum number of steps, but I can provide this summary of the findings."""
        ...
        try:
            ...
            try:
                ...
                if cleaned_content.strip().startswith('{') and not cleaned_content.strip().endswith('}'):
                    ...
                    import re
                    match = re.search(r'\{[^}]*"tool"[^}]*"parameters"[^}]*\}', cleaned_content)
                    if match:
                        try:
            ...
            if action_content.startswith('{') and not action_content.endswith('}'):
                ...
                logger.warning("Could not recover from incomplete JSON, skipping action")
                return None, None
                ...
                if match:
                    try:
                        ...
                        try:
                            ...
                            logger.debug(f"Failed to parse tool input: JSON error: {e1}, Eval error: {e2}")
                            continue  # Try next pattern
            
            # Additional pattern for legacy "Input:" without braces
            if not tool_input:
                legacy_input_match = re.search(r'Input:\s*([^{][^\n]*)', action_content, re.IGNORECASE)
    ...
    def _fallback_action_parse(self, action_content: str) -> Tuple[Optional[str], Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            for tool_name in tool_names:
                if tool_name.lower() in action_content.lower():
                    logger.info(f"Found tool name '{tool_name}' in malformed action, using with empty parameters")
                    return tool_name, {}
            
            # If no tools found, treat as non-tool action
            logger.info("No tool detected in malformed action, treating as non-tool action")
            return None, None
        except Exception as e:
