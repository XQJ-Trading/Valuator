...
class MongoSessionRepository(SessionRepository):
    ...
    def _init_connection(self):
        ...
        try:
            ...
            logger.info(f"MongoDB connection established: {self.database_name}.{self.collection_name}")
    ...
    async def list_sessions(
        ...
        try:
            ...
            logger.debug(f"Listed {len(sessions)} sessions from MongoDB (limit={limit}, offset={offset})")
            return sessions
    ...
    async def search_sessions(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Run MongoDB operation in thread pool
            cursor = await asyncio.to_thread(
                lambda: self.collection.find(search_filter)
                    .sort("created_at", DESCENDING)
            )
            ...
            logger.debug(f"Found {len(sessions)} sessions in MongoDB matching query: {query}")
            return sessions
    ...
    async def delete_session(self, session_id: str) -> bool:
        ...
        try:
            ...
            if result.deleted_count > 0:
                ...
                logger.warning(f"Session not found for deletion in MongoDB: {session_id}")
                return False
