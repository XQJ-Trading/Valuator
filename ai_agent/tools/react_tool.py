...
class PerplexitySearchTool(ReActBaseTool):
    ...
    async def _execute_impl(self, query: str) -> ToolResult:
        ...
        try:
            ...
            # 시스템 프롬프트와 사용자 쿼리 구성
            messages = [
                SystemMessage(content="You are a helpful search assistant. Provide accurate, up-to-date information with sources when possible. Be concise but comprehensive."),
...
class CodeExecutorTool(ReActBaseTool):
    ...
    async def _execute_impl(self, code: str, timeout: int = None, language: str = None) -> ToolResult:
        ...
        try:
            ...
            if language and language.lower() != 'python':
                self.logger.warning(f"Language '{language}' specified but only Python is supported")
...
class FileSystemTool(ReActBaseTool):
    ...
    async def _execute_impl(self, operation: str, path: str, content: str = None, **kwargs) -> ToolResult:
