...
def langchain_to_google_messages(messages: List[BaseMessage]) -> List[Dict[str, Any]]:
    ...
    for content in contents:
        if not content.get('parts') or not any(part.strip() for part in content['parts'] if isinstance(part, str)):
...
def google_to_langchain_message(response, extract_thinking: bool = True) -> AIMessage:
    ...
    if hasattr(response, 'usage_metadata'):
        ...
        if hasattr(usage, 'cached_content_token_count'):
            usage_metadata['cached_content_token_count'] = usage.cached_content_token_count
    ...
    if metadata:
        if not hasattr(message, 'response_metadata') or message.response_metadata is None:
