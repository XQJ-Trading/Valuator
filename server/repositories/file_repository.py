...
class FileSessionRepository(SessionRepository):
    ...
    async def list_sessions(
        ...
        try:
            ...
            files = await asyncio.to_thread(
                lambda: list(self.logs_dir.glob("*.json"))
            )
            ...
            logger.debug(f"Listed {len(sessions)} sessions (limit={limit}, offset={offset})")
            return sessions
    ...
    async def search_sessions(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Found {len(matching_sessions)} sessions matching query: {query}")
