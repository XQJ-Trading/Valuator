...
- **root** [waiting] Analysis: 플래닛랩스 분석해줘
...
- 공시 자료(사업보고서 등)를 기반으로 기업의 모든 사업부문을 명확히 구분하고 정의합니다.
...
[THINKING_LEVEL]
low
  - **root.0** [done] 플래닛랩스의 공시 자료를 기반으로 최근 3~5년간 사업 부문별 매출액 추이와 영업이익률 등 수익성 기여도를 분석합니다.
  - **root.1** [waiting] 위성 이미지 산업의 전방 수요(정부, 상업용)와 밸류체인을 분석하고, 차년도 실적에 영향을 미칠 긍정적 요인과 리스크를 전망합니다.
    - **root.1.0** [done] Planet Labs의 정부 및 상업용 전방 수요 동향과 주요 고객군 수주 잔고 분석 (leaf: web_search_tool)
    - **root.1.1** [running] 위성 데이터 수집, 처리, 서비스 제공에 이르는 밸류체인 내 Planet Labs의 위치와 경쟁력 분석 (leaf: domain_tool)
    - **root.1.2** [waiting] 차년도 실적 성장을 견인할 긍정적 촉매제(신규 위성 발사, 계약 갱신 등)와 하방 리스크(거시 경제, 경쟁 심화) 분석
      - **root.1.2.0** [running] Planet Labs의 차년도 실적 성장을 견인할 긍정적 촉매제(Pelican/Tanager 위성 발사 계획, 주요 정부 및 상업용 계약 갱신, 신규 제품 출시 등) 분석 (leaf: web_search_tool)
      - **root.1.2.1** [done] Planet Labs의 주요 하방 리스크(경쟁사 동향, 거시 경제 영향)와 요구사항에 명시된 재무 건전성(부채비율, 현금 흐름) 및 지배구조 투명성 리스크 분석 (leaf: web_search_tool)
  - **root.2** [waiting] 부채비율, 현금흐름 건전성 등 재무 지표와 이사회 독립성, 주요 주주 구성 등 지배구조 리스크를 검토합니다.
    - **root.2.0** [running] Planet Labs (PL)의 최근 3개년 부채비율, 유동비율 및 영업/투자/재무 활동별 현금흐름 추이를 분석하여 재무적 안정성과 자금 조달 현황을 파악합니다. (leaf: yfinance_balance_sheet)
    - **root.2.1** [done] Planet Labs (PL)의 최신 프록시 서류(DEF 14A) 및 공시를 바탕으로 이사회 독립성, 주요 주주 구성(기관 및 창업자 지분율), 의결권 구조 등 지배구조 리스크를 검토합니다. (leaf: web_search_tool)
  - **root.3** [created] 이전 단계의 분석 결과를 종합하여 강점(Bull)과 약점(Bear)을 정리하고 최종 투자 전망을 제시합니다.
