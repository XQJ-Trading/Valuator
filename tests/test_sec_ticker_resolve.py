...
from domain.company import listing_seed_from_sec_record, resolve_subjects
...
def test_listing_seed_from_sec_record_builds_usa_listing() -> None:
    seed = listing_seed_from_sec_record(
...
def test_resolve_by_ticker(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "domain.boundary.sec_ticker_resolve.fetch_sec_ticker_records",
        lambda **_: rows,
    )
    seeds = resolve_sec_listing_seeds("ABCD")
    assert len(seeds) == 1
...
def test_resolve_by_ticker_class_share_dot(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    seeds = resolve_sec_listing_seeds("BRK.B")
    assert len(seeds) == 1
...
def test_exact_title_duplicate_returns_empty(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert resolve_sec_listing_seeds("Same Name Inc") == ()
...
def test_sec_on_miss_is_resolve_alias(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert sec_on_miss("ZZ") == resolve_sec_listing_seeds("ZZ")
