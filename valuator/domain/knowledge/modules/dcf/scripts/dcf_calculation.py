...
for _, scenario in scenarios.items():
    if base_enterprise_value == 0:
        ...
        scenario["ev_vs_base_pct"] = (
            (scenario["enterprise_value"] / base_enterprise_value) - 1.0
        ) * 100.0

wacc_offsets = [-0.01, 0.0, 0.01]
growth_offsets = [-0.01, 0.0, 0.01]
wacc_values = [
    clamp(base_case_params["discount_rate"] + offset, 0.001, 1.5)
    for offset in wacc_offsets
]
growth_values = [
    clamp(base_case_params["growth_rate"] + offset, -0.95, 2.0)
...
wacc_max_abs_delta = max(
    abs(wacc_down_ev - base_enterprise_value),
    abs(wacc_up_ev - base_enterprise_value),
)
growth_max_abs_delta = max(
    abs(growth_down_ev - base_enterprise_value),
    abs(growth_up_ev - base_enterprise_value),
)
most_impactful_variable = (
    "discount_rate"
    if wacc_max_abs_delta >= growth_max_abs_delta
    else "growth_rate"
)
